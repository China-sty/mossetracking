...
class mosse:
    def __init__(self, args, img_path):
        # get arguments..
        self.args = args
        self.img_path = os.path.join('datasets/test_public',img_path)
        self.vname=img_path
        # get the img lists...
    ...
    def start_tracking(self):
        ...
        pr=[]
        init_gt = []
        gt_txt = 'datasets/gt'+'/'+self.vname+'.txt'
        with open(gt_txt, 'r') as f:
            for line in f.readlines():
                line = list(map(float, line.strip().split(',')))
                init_gt.append(list(map(int, line)))
        bbox = init_gt[0]
        if(bbox[4] - bbox[0]<=0):
            bbox = [bbox[0], bbox[1], bbox[0]-bbox[4], bbox[5] - bbox[1]]
        elif(bbox[5] - bbox[1]<=0):
            bbox = [bbox[0], bbox[1], bbox[4] - bbox[0], bbox[1] - bbox[5]]
        else:
        ...
        for idx in range(len(self.frame_lists)):
           ...
           # print((pos[0], pos[1]), (pos[0]+pos[2], pos[1]+pos[3]))
            pr.append([pos[0],pos[1],pos[0]+pos[2],pos[0],pos[0]+pos[2],pos[1]+pos[3],pos[0],pos[1]+pos[3]])

            #write(pos[0],pos[1],pos[0]+pos[2],pos[1]+pos[3])
    ...
    def write(self,pr):
        dr='datasets/pre/'+self.vname+'.txt'
