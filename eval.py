...
def load_file(zipfile, fname):
    ...
    for line in content:
        ...
        x1, y1, x2, _, _, y2, _, _ = line.split(',')
        x1 = float(x1)
        y1 = float(y1)
        x2 = float(x2)
        y2 = float(y2)
...
def evaluate(test_annotation_files, user_submission_files):
    archive_anno = zipfile.ZipFile(test_annotation_files, 'r')
    archive_subm = zipfile.ZipFile(user_submission_files, 'r')
    ...
    for i, name_file_anno in tqdm(enumerate(otb_files_anno)):
        file = load_file(archive_anno, name_file_anno)
        df = pd.DataFrame({"anno_x1":file['x1'], "anno_y1":file['y1'], "anno_w":file['w'], "anno_h":file['h']})

        name_file_subm = [s for s in otb_files_subm if os.path.basename(name_file_anno) in s]

        if (len(name_file_subm) == 1):
            file_subm = load_file(archive_subm, name_file_subm[0])
            df_subm = pd.DataFrame({"subm_x1": file_subm['x1'], "subm_y1": file_subm['y1'], "subm_w": file_subm['w'], "subm_h": file_subm['h']})
            df["sanity_check"] = 1.0
        else:
        ...
        Success_test_avg += np.array(
            [np.sum(i >= thres for i in df["IoU"]).astype(float) / (len(df["IoU"])) for thres in Success_X])
        Precision_test_avg += np.array(
            [np.sum(i <= thres for i in df["distance"]).astype(float) / (len(df["distance"])) for thres in Precision_X])
        NPrecision_test_avg += np.array(
            [np.sum(i <= thres for i in df["ndistance"]).astype(float) / (len(df["ndistance"])) for thres in
             NPrecision_X])
...
if __name__ == "__main__":
    ...
    p.add_argument('--GT_zip', type=str, default='datasets/gt.zip',
                   help='zipped folder with GT OTB tracking bounding boxes.')
    p.add_argument('--subm_zip', type=str, default='datasets/pre.zip',
